...
##### DEBUGed: Fruit flags & servings (keeps solids that are "packed in juice")-----
...
#####  DEBUGed: Vegetable flags & servings ------
veg_flags <- iff_base %>%
  dplyr::mutate(
...
other_person <- iff_base %>%
  dplyr::mutate(
    ...
    ## 🔥🔥🔥🔥🔥🔥🔥
    # ---------- helpers ----------
    dish_pat = "(?i)(sandwich|burger|on\\s*(a\\s*)?bun|wrap|pita|stew|with\\s*(starch(es)?|potato(es)?|rice|pasta|noodle(s)?|vegetable(s)?)|frozen|tv\\s*dinner|meal|soup)",
    ...
    # GUARD 2 (wider “chunky” cues so sliced/diced/chopped meat is kept)
    chunky_tokens = "(?i)(chunks?|pieces?|strips?|shreds?|cubes?|ground|meatballs?|patty|patties|ribs?|slices?|sliced|diced|chopped|minced)",
    
    # GUARD 3 (tightened): only treat as “flavor-only” if it’s clearly an instant/packet soup/noodle
    ...
    # ---------- burgers ----------
    burger_beef = stringr::str_detect(dl, "(?i)\\b(cheese[-\\s]*)?burger(s)?\\b") &
      !stringr::str_detect(dl, "(?i)(vegan|veggie|plant[-\\s]*based|soy|black[-\\s]*bean|impossible|beyond|turkey|chicken|fish|salmon).*burger"),
    
    # processed add-ons inside burgers (e.g., bacon cheeseburger) → processed
    ...
    burger_proc = burger_beef & stringr::str_detect(dl, proc_addon_pat),
    
    # ---------- beef ----------
    beef_any  = stringr::str_detect(dl, "(?i)\\bbeef\\b|\\bhamburger\\b") | burger_beef,
    
    beef_core = stringr::str_detect(
      dl,
      ...
      # safe recall: generic “steak” defaults to beef unless clearly non-beef
      ( stringr::str_detect(dl, "(?i)\\bsteaks?\\b") &
          !stringr::str_detect(dl, "(?i)\\b(tuna|salmon|ahi|swordfish|mahi|fish|cauliflower|portobello|mushroom|tofu|seitan|tempeh|pork|chicken|turkey)\\b") ),
    
    # a few high-signal beef aliases
    beef_alias_mix = stringr::str_detect(
    ...
    beef_liquid_only = beef_any &
      stringr::str_detect(dl, liquid_tokens) &
      !stringr::str_detect(dl, chunky_tokens),
    
    beef_flavor_only = beef_any &
      stringr::str_detect(dl, flavor_env) &
      stringr::str_detect(dl, flavor_words),
    
    beef_mix  = (beef_any | beef_alias_mix) &
      ( stringr::str_detect(dl, dish_pat) | stringr::str_detect(dl, baby_food_pat) ) &
      !beef_liquid_only & !beef_flavor_only & !(burger_beef & burger_proc),
    
    # ---------- pork ----------
    pork_any  = stringr::str_detect(dl, "(?i)\\bpork\\b|\\bham\\b|\\bsparerib(s)?\\b") |
      stringr::str_detect(dl, "(?i)\\b(carnitas|al\\s*pastor|char\\s*siu|kalua\\s*pork|lechon|porchetta|chashu)\\b"),
    
    pork_core = stringr::str_detect(
      dl,
      ...
      # safe recall: generic “ribs/spare ribs” default to pork if species not stated
      ( stringr::str_detect(dl, "(?i)\\b(ribs?|spare\\s*ribs?)\\b") &
          !stringr::str_detect(dl, "(?i)\\b(beef|lamb|veal|goat|chicken|turkey|fish|seafood)\\b") ),
    
    pork_liquid_only = pork_any &
      stringr::str_detect(dl, liquid_tokens) &
      !stringr::str_detect(dl, chunky_tokens),
    
    pork_flavor_only = pork_any &
      stringr::str_detect(dl, flavor_env) &
      stringr::str_detect(dl, flavor_words),
    
    pork_mix  = pork_any &
      ( stringr::str_detect(dl, dish_pat) | stringr::str_detect(dl, baby_food_pat) ) &
      !pork_liquid_only & !pork_flavor_only & !(burger_beef & burger_proc),
    
    # ---------- other red meats ----------
    other_red_any  = stringr::str_detect(dl, "(?i)\\b(lamb|mutton|veal|goat)\\b") |
      ( stringr::str_detect(dl, "(?i)\\b(gyro|doner|shawarma|souvlaki)\\b") &
          !stringr::str_detect(dl, "(?i)\\b(chicken|turkey)\\b") ),
    other_red_core = stringr::str_detect(dl, "(?i)\\b(lamb|mutton|veal|goat)\\s*(chops?|steak|roast|rib(s)?|shank|loin)\\b"),
    other_red_mix  = other_red_any & stringr::str_detect(dl, dish_pat),
    ...
    proc_core = stringr::str_detect(
      dl,
      paste0("(?i)",
             "\\bham\\b|canadian\\s*bacon|\\bbacon\\b|salt\\s*pork|pork\\s*skin|",
             "\\bfrankfurters?\\b|\\bwieners?\\b|\\bhot\\s*dogs?\\b|\\bsausages?\\b|\\bbratwurst\\b|\\bkielbasa\\b|\\bchorizo\\b|",
             "luncheon\\s*meat|lunch\\s*meat(s)?|cold[-\\s]*cut(s)?|deli\\s*meat(s)?|",
             "\\bbologna\\b|\\bbaloney\\b|\\bsalami\\b|\\bpepperoni\\b|\\bpastrami\\b|corned\\s*beef|\\bliverwurst\\b|",
      ...
      stringr::str_detect(dl, "(?i)\\b(beef|pork|turkey|chicken)\\s+franks?\\b") |
      stringr::str_detect(dl, named_proc_sandwich),
    ...
    proc_half = proc_half | ( stringr::str_detect(dl, "(?i)\\b(charcuterie|antipasto(\\s*platter)?|salumi)\\b") & !proc_core ),
    proc_half = proc_half | stringr::str_detect(dl, "(?i)\\bbacon[-\\s]*wrapped\\b"),
    ...
    # ---------- final flags (exclude processed to avoid double count) ----------
...
misses <- iff_base %>%
  mutate(flag_any = (beef_full|beef_half|pork_full|pork_half|other_red_full|other_red_half|proc_core|proc_half),
         meatish  = str_detect(dl, "(?i)\\b(beef|hamburger|steak|rib|ribs|pork|ham|bacon|sausage|pepperoni|salami|pastrami|prosciutto|kielbasa|chorizo|frank|hot\\s*dog|corned\\s*beef|brisket)\\b")) %>%
  filter(meatish & !flag_any) %>%
  count(dl, sort = TRUE)
