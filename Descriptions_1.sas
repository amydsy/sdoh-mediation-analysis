...
%chisq (FS, sex race edu pir SNAP smk alcg2 
bmic diabe CVD dm_rx chol_rx	angina_rx cancer lung_disease  mortstat death_cancer 
death_cvd death_cmd death_heart death_cerev , score_mort, Descrb1a) ;

%chisq (pir, sex race edu FS snap smk alcg2 
bmic diabe CVD dm_rx chol_rx	angina_rx cancer lung_disease  mortstat death_cancer 
death_cvd death_cmd death_heart death_cerev , score_mort, Descrb1b) ;

%chisq (SNAP, sex race edu FS PIR smk alcg2
bmic diabe CVD dm_rx chol_rx	angina_rx cancer lung_disease  mortstat death_cancer 
death_cvd death_cmd death_heart death_cerev , score_mort, Descrb1c) ;

%chisq (i_FCS_sdq, sex race edu PIR  FS SNAP smk alcg2 
bmic diabe CVD dm_rx chol_rx	angina_rx cancer lung_disease  mortstat death_cancer 
death_cvd death_cmd death_heart death_cerev , score_mort1, Descrb1d) ;
PROC EXPORT DATA= WORK.DESCRB1a 
            OUTFILE= "&home\DESCRB1a.csv" 
            DBMS=CSV REPLACE;
     PUTNAMES=YES;
RUN;
PROC EXPORT DATA= WORK.DESCRB1b 
            OUTFILE= "&home\DESCRB1b.csv" 
            DBMS=CSV REPLACE;
     PUTNAMES=YES;
RUN;
PROC EXPORT DATA= WORK.DESCRB1c 
            OUTFILE= "&home\DESCRB1ca.csv" 
            DBMS=CSV REPLACE;
     PUTNAMES=YES;
RUN;
PROC EXPORT DATA= WORK.DESCRB1d 
            OUTFILE= "&home\DESCRB1d.csv" 
            DBMS=CSV REPLACE;
     PUTNAMES=YES;
RUN;
...
var &dvar ;
data ab ; set d1 d2 ; 
&dvar= compress(round(mean,0.01)||"("||round(stderr,.01)||")") ;  run ;
...
%Anova( FS,ridageyr met_hr BMI hba1c sbp	dbp  hdl ldl tg hei2015_TOTAL_SCORE , score_mort, Descrb1)


%Anova( pir,ridageyr met_hr BMI hba1c sbp	dbp  hdl ldl tg hei2015_TOTAL_SCORE ,score_mort, Descrb2)

%Anova( SNAP,ridageyr met_hr BMI hba1c sbp	dbp  hdl ldl tg hei2015_TOTAL_SCORE ,score_mort, Descrb3)

%Anova( i_FCS_sdq,ridageyr met_hr hba1c sbp	dbp  hdl ldl tg BMI hei2015_TOTAL_SCORE ,score_mort1, Descrb4)


PROC EXPORT DATA= WORK.DESCRB1 
            OUTFILE= "&home\DESCRB1.csv" 
            DBMS=CSV REPLACE;
     PUTNAMES=YES;
RUN;
PROC EXPORT DATA= WORK.DESCRB2 
            OUTFILE= "&home\DESCRB2.csv" 
            DBMS=CSV REPLACE;
     PUTNAMES=YES;
RUN;

PROC EXPORT DATA= WORK.DESCRB3 
            OUTFILE= "&home\DESCRB3.csv" 
            DBMS=CSV REPLACE;
     PUTNAMES=YES;
RUN;


PROC EXPORT DATA= WORK.DESCRB4 
            OUTFILE= "&home\DESCRB4.csv" 
            DBMS=CSV REPLACE;
     PUTNAMES=YES;
RUN;


 FS ridageyr sex race edu pir ridageyr sex race edu smk	met_hr perE_alco 
CVD dm_rx chol_rx	angina_rx Cancer lung_disease angina bmi	 pir 
